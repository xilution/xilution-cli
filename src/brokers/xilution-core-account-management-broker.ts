...
import {v4} from "uuid";
import {buildOAuthImpersonationTokenUrl} from "./xilution-core-authentication-broker";

export const buildListProductActivationsUrl = (env: string, pageNumber: number, pageSize: number) =>
...
export const listProductActivations = async (
    ...
    pageNumber: number,
    pageSize: number,
): Promise<AxiosResponse> => axios.get(buildListProductActivationsUrl(env, pageNumber, pageSize), {
    headers: {
...
export const activateProduct = async (
    ...
    accessToken: string,
    productId: string,
): Promise<AxiosResponse> => axios.post(buildActivateProductUrl(env, productId), undefined, {
    headers: {
        Authorization: `Bearer ${accessToken}`,
    },
    validateStatus: () => true,
...
export const deactivateProduct = async (
    ...
    accessToken: string,
    productId: string,
): Promise<AxiosResponse> => axios.post(buildDeactivateProductUrl(env, productId), undefined, {
    headers: {
        Authorization: `Bearer ${accessToken}`,
    },
    validateStatus: () => true,
